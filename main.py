...
class FaceApp(App):
    ...
    def build(self):  # noqa: D401 (Kivy signature)
        ...
        # Open webcam (index 0).
        # On most mobile devices, index 0 often corresponds to the front-facing camera.
        # If you need to explicitly try other cameras, you might iterate or use platform-specific APIs.
        self.capture = cv2.VideoCapture(0)
        if not self.capture.isOpened():
            # If camera cannot be opened, raise an error to inform the user/developer.
    ...
    def _load_emails(self) -> Dict[str, str]:
        """Loads stored user email addresses from a JSON file in internal storage."""
    ...
    def _save_email(self, emp_id: str, email: str) -> None:
        """Saves a user's email address to the JSON file in internal storage."""
        self.user_emails[emp_id] = email
        with (self._known_faces_dir / "user_emails.json").open("w", encoding="utf-8") as f:
            json.dump(self.user_emails, f, indent=2) # Save with indentation for readability.
    ...
    def _overlay_tick_next_to_name(self, frame: np.ndarray, text_x: int, text_y_baseline: int, name: str, emp_id: str, font_scale: float, font_thickness: int) -> None:
        ...
        # Get the size of the text to correctly position the tick icon next to it.
        (text_width, text_height), _ = cv2.getTextSize(text_to_measure, cv2.FONT_HERSHEY_SIMPLEX, font_scale, font_thickness)

        # Desired fixed size for the tick icon (e.g., 25x25 pixels)
        tick_icon_size = 25 # pixels
        # Resize the tick icon to the desired size using INTER_AREA for shrinking.
        ...
        # Ensure the icon is within frame boundaries to prevent drawing outside the image.
        ...
        if roi.shape[0] == tick_icon_size and roi.shape[1] == tick_icon_size:
            if icon.shape[2] == 4:  # If icon has an alpha channel (RGBA).
                ...
                mask = cv2.merge((a, a, a)) / 255.0 # Create 3-channel mask from alpha.
                # Blend the icon onto the ROI using the alpha mask.
                ...
                # Fallback for RGB images: simple copy (less ideal for overlays).
